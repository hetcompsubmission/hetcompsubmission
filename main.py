...
def DKC(sorted_mat, last_max_idx, last_dist, is_first, epoch, alpha=1.05):
	...
	for model_idx, model_type in enumerate(perm_dict):
		for user in range(user_count):
			...
			if ((last_dist[model_idx][user] / next_dist) > th):
...
state_dict, t_results, p_results = load_teacher_trajectory(path, model_list)
...
for epoch in range(1000):
	...
	if epoch % verbose == 0:
		...
		with torch.no_grad():
			...
			print_result(epoch, 1000, ep_loss, e_results, imp=imp, train_time=toc1-tic1, test_time=toc2-tic2)
	...
	if (epoch % p == 0) and (epoch >= 10) and v_results.sum() < 18:
		...
		t_results = g_torch(state_dict, v)
